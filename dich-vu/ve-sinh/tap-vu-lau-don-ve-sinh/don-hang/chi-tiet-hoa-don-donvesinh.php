...
<style>
	:root {
		...
		--danger: #c93b3b;
		--border: #f0d4e3;
		--shadow: 0 12px 26px rgba(90, 122, 228, 0.12), 0 2px 8px rgba(191, 200, 219, 0.1);
		--radius-xl: 22px;
		--radius-lg: 16px;
		--radius-md: 12px;
	...
	* {
	...
	.modal-card {
		...
		box-shadow: 0 24px 48px rgba(90, 122, 228, 0.18), 0 6px 20px rgba(138, 170, 209, 0.12);
	...
	.invoice-hero {
		...
		padding: 16px;
		color: #fff;
		background: linear-gradient(118deg, #5a7ae4 0%, #69a0f3 48%, #86b0ef 72%, #a9cbff 100%);
	}
	...
	.invoice-progress-ring {
		...
		box-shadow: inset 0 0 0 1px rgba(255, 255, 255, 0.22), 0 10px 22px rgba(0, 0, 0, 0.2);
		padding: 7px;
		flex: 0 0 auto;
	}
	...
	.invoice-progress-core {
		...
		background:
			...
			linear-gradient(150deg, #69a0f3 0%, #5a7ae4 100%);
	...
	/* Bootstrap Overrides */
	.btn.btn-primary {
		background: linear-gradient(135deg, #5a7ae4 0%, #3b50c9 100%) !important;
		color: #fff !important;
	...
	.appDialogBtnPrimary {
		background: linear-gradient(135deg, #8079e7, #4611c2) !important;
		color: white;
		box-shadow: 0 4px 12px rgba(17, 88, 194, 0.3);
	}
			...
			<section id="mainGrid" class="grid hidden">
				...
				<article class="panel" id="panelCustomer">
					...
					<div class="profile-body">
						<img id="customerAvatarImg" class="profile-avatar" src="../assets/logo_main.png" alt="Khách hàng">
				...
				<article class="panel" id="panelStaff">
					...
					<div class="profile-body">
						<img id="staffAvatarImg" class="profile-avatar" src="../assets/logo_main.png" alt="Nhà Cung Cấp">
...
<script>
	(() => {
		...
		const TABLE_ORDER = 'datlich_donvesinh';
		...
		const setAvatar = (id, src, fallback = '../assets/logo_main.png') => {
			const el = $(id);
			if (!el) return;
			el.onerror = () => {
				el.onerror = null;
		...
		function findAuthUser() {
			...
			if (!app.$datlich_donvesinh) return null;

			const sdt = sodienthoai.trim();
			const customerPhone = String(app.$datlich_donvesinh.sdtkhachhang || '').trim();
			const sdtncc = String(app.$datlich_donvesinh.sdtncc || '').trim();
		...
		async function applyAutoOrderStatusRules($datlich_donvesinh) {
			const status = String($datlich_donvesinh.trangthai || '').toLowerCase();
			const hasStaff = toNum($datlich_donvesinh.id_nhacungcap) > 0;
			const startPlanAt = buildPlanDateTime($datlich_donvesinh.ngay_bat_dau_kehoach, $datlich_donvesinh.gio_bat_dau_kehoach);
			const endPlanAt = buildPlanDateTime($datlich_donvesinh.ngay_ket_thuc_kehoach, $datlich_donvesinh.gio_ket_thuc_kehoach, true);
			...
			if (needOverdue) {
				await updateById(TABLE_ORDER, $datlich_donvesinh.id, { trangthai: 'quá hạn' });
				return { ...$datlich_donvesinh, trangthai: 'quá hạn' };
			}

			if (needComplete) {
				await updateById(TABLE_ORDER, $datlich_donvesinh.id, { trangthai: 'hoàn thành' });
				return { ...$datlich_donvesinh, trangthai: 'hoàn thành' };
			}

			return $datlich_donvesinh;
		}

		async function loadOrderById() {
			const $datlich_donvesinh_rows = await listTableRows(TABLE_ORDER);
			const $datlich_donvesinh = $datlich_donvesinh_rows.find((row) => String(row.id) === String(app.params.mahd));
			if (!$datlich_donvesinh) throw new Error('Không tìm thấy đơn hàng');

			app.$datlich_donvesinh = await applyAutoOrderStatusRules($datlich_donvesinh);
		}

		function getAssignedStaffRow() {
			if (!app.$datlich_donvesinh) return null;
			const idStaff = toNum(app.$datlich_donvesinh.id_nhacungcap);
		...
		function calcProgressDay($datlich_donvesinh) {
			const plan = parseTimeToHour($datlich_donvesinh.gio_ket_thuc_kehoach) - parseTimeToHour($datlich_donvesinh.gio_bat_dau_kehoach);
			const realStart = new Date($datlich_donvesinh.thoigian_batdau_thucte || '').getTime();
			const realEnd = new Date($datlich_donvesinh.thoigian_ketthuc_thucte || '').getTime();
			const real = (realEnd - realStart) / 3600000;
			const days = diffDays($datlich_donvesinh.ngay_bat_dau_kehoach, $datlich_donvesinh.ngay_ket_thuc_kehoach);
		...
		function isReviewLockedByDeadline($datlich_donvesinh) {
			if (!$datlich_donvesinh) return true;
			const status = String($datlich_donvesinh.trangthai || '').toLowerCase();
			const endPlanAt = buildPlanDateTime($datlich_donvesinh.ngay_ket_thuc_kehoach, $datlich_donvesinh.gio_ket_thuc_kehoach, true);
			...
			const isCanceled = status.includes('hủy');
			const isOverdue = status.includes('quá hạn');
			const noStaff = toNum($datlich_donvesinh.id_nhacungcap) <= 0;
			return now().getTime() > endPlanAt.getTime() && (isCanceled || isOverdue || noStaff);
		}

		function canSubmitCustomerReview() {
			const $datlich_donvesinh = app.$datlich_donvesinh;
			if (!$datlich_donvesinh) return false;
			const isCompleted = String($datlich_donvesinh.trangthai || '').toLowerCase().includes('hoàn thành');
			return isCompleted && !isReviewLockedByDeadline($datlich_donvesinh);
		}
		...
		function canSubmitStaffReview() {
			...
			if (!$datlich_donvesinh) return false;
			const isCompleted = String($datlich_donvesinh.trangthai || '').toLowerCase().includes('hoàn thành');
			const isAssignedStaff = toNum($datlich_donvesinh.id_nhacungcap) > 0
				&& toNum($datlich_donvesinh.id_nhacungcap) === toNum(app.$user?.id);
			return isCompleted && isAssignedStaff && !isReviewLockedByDeadline($datlich_donvesinh);
		}
		...
		function bindReviewPreview(inputId, gridId, mediaColumn) {
			...
			input.addEventListener('change', () => {
				...
				renderMedia(gridId, (app.$datlich_donvesinh || {})[mediaColumn]);
			});
		}

		function renderReview() {
			const $datlich_donvesinh = app.$datlich_donvesinh;
			if (!$datlich_donvesinh) return;
			const customerDone = !!$datlich_donvesinh.danhgia_khachhang;
			const staffDone = !!$datlich_donvesinh.danhgia_nhanvien;

			setText('customerReviewText', $datlich_donvesinh.danhgia_khachhang || 'Chưa có đánh giá');
			setText('customerReviewTime', formatDateTimeDisplay($datlich_donvesinh.thoigian_danhgia_khachhang));
			setText('staffReviewText', $datlich_donvesinh.danhgia_nhanvien || 'Chưa có đánh giá');
			setText('staffReviewTime', formatDateTimeDisplay($datlich_donvesinh.thoigian_danhgia_nhanvien));
			renderMedia('customerReviewMediaGrid', $datlich_donvesinh.media_danhgia_khachhang);
			renderMedia('staffReviewMediaGrid', $datlich_donvesinh.media_danhgia_nhanvien);
		...
		function renderUI() {
			...
			const invoiceCode = formatInvoiceCode($datlich_donvesinh.id || app.params.mahd);

			setText('invoiceId', invoiceCode);
			setText('invoicePrice', formatMoney($datlich_donvesinh.tong_tien || 0));
			setText('invoiceService', $datlich_donvesinh.dich_vu || '---');
			setText('invoiceLoaiNoiDon', $datlich_donvesinh.loai_noi_don || '---');
			setText('invoiceGoiDichVu', $datlich_donvesinh.goi_dich_vu || '---');
			setText('invoiceAddress', $datlich_donvesinh.diachikhachhang || '---');
			setText('invoiceTimeRange', `${formatTimeDisplay($datlich_donvesinh.gio_bat_dau_kehoach)} - ${formatTimeDisplay($datlich_donvesinh.gio_ket_thuc_kehoach)}`);
			setText('invoiceDate', `${formatDateDisplay($datlich_donvesinh.ngay_bat_dau_kehoach)}${$datlich_donvesinh.ngay_ket_thuc_kehoach ? ` -> ${formatDateDisplay($datlich_donvesinh.ngay_ket_thuc_kehoach)}` : ''}`);
			setText('invoiceRequest', $datlich_donvesinh.yeu_cau_khac || '---');
			setText('invoiceNote', $datlich_donvesinh.ghi_chu || '---');
			renderJobList($datlich_donvesinh.cong_viec || '');
			...
			renderDriveFrame($datlich_donvesinh.anh_id || '', 'invoiceMediaImageEl', 'invoiceMediaImageEmpty');
			renderDriveFrame($datlich_donvesinh.video_id || '', 'invoiceMediaVideoEl', 'invoiceMediaVideoEmpty');

			setText('customerCardName', $datlich_donvesinh.tenkhachhang || '---');
			setText('customerCardPhone', $datlich_donvesinh.sdtkhachhang || '---');
			setText('customerCardEmail', $datlich_donvesinh.emailkhachhang || '---');
			setText('customerCardAddress', $datlich_donvesinh.diachikhachhang || '---');
			...
			updateAvatarIframe('customerAvatar', $datlich_donvesinh.avatar_khachhang);

			setText('staffCardName', $datlich_donvesinh.tenncc || '---');
			setText('staffCardEmail', $datlich_donvesinh.emailncc || '---');
			setText('staffCardPhone', $datlich_donvesinh.sdtncc || '---');
			setText('staffCardAddress', $datlich_donvesinh.diachincc || '---');
			setText('staffReceiveTime', `Nhận việc: ${formatDateTimeDisplay($datlich_donvesinh.ngaynhan)}`);
			updateAvatarIframe('staffAvatar', $datlich_donvesinh.avatar_ncc);

			const status = $datlich_donvesinh.trangthai || '---';
			...
			setText('timeStartPlan', `${formatTimeDisplay($datlich_donvesinh.gio_bat_dau_kehoach)}`.trim());
			setText('timeEndPlan', `${formatTimeDisplay($datlich_donvesinh.gio_ket_thuc_kehoach)}`.trim());
			setText('timeStartReal', formatDateTimeDisplay($datlich_donvesinh.thoigian_batdau_thucte));
			setText('timeEndReal', formatDateTimeDisplay($datlich_donvesinh.thoigian_ketthuc_thucte));

			const progress = Math.max(0, Math.min(100, toNum($datlich_donvesinh.tien_do || 0)));
			...
			const totalDayCount = Math.max(1, diffDays($datlich_donvesinh.ngay_bat_dau_kehoach, $datlich_donvesinh.ngay_ket_thuc_kehoach) + 1);
			...
			setText('dateStartPlan', formatDateDisplay($datlich_donvesinh.ngay_bat_dau_kehoach));
			setText('dateEndPlan', formatDateDisplay($datlich_donvesinh.ngay_ket_thuc_kehoach));
			...
			$('badgeStaff').textContent = toNum($datlich_donvesinh.id_nhacungcap) > 0 ? 'Đã nhận' : 'Chưa nhận';
			$('badgeStaff').className = toNum($datlich_donvesinh.id_nhacungcap) > 0 ? 'profile-status badge success' : 'profile-status badge warning';
		...
		function renderWorkHistoryTable() {
			...
			const endPlanTime = app.$datlich_donvesinh ? app.$datlich_donvesinh.gio_ket_thuc_kehoach : '';
		...
		async function updateCustomerAvatarIfNeeded() {
			const order = app.$datlich_donvesinh;
			const user = app.$user;
		...
		async function handleClaim() {
			const $datlich_donvesinh = app.$datlich_donvesinh || {};
			const statusText = String($datlich_donvesinh.trangthai || '').toLowerCase();
			...
			if (startPlanAt && now().getTime() >= startPlanAt.getTime()) {
				if (toNum($datlich_donvesinh.id_nhacungcap) <= 0 && !statusText.includes('hủy') && !statusText.includes('hoàn thành')) {
					await updateById(TABLE_ORDER, $datlich_donvesinh.id, { trangthai: 'quá hạn' });
					await reloadOrder();
				}
				throw new Error('Đơn hàng quá hạn, không thể nhận việc');
			...
			await updateById(TABLE_ORDER, app.$datlich_donvesinh.id, {
		...
		async function handleCancel() {
			...
			if (startPlanAt && now().getTime() >= startPlanAt.getTime()) {
				if (!hasStaff && !statusText.includes('hoàn thành')) {
					await updateById(TABLE_ORDER, app.$datlich_donvesinh.id, { trangthai: 'quá hạn' });
					await reloadOrder();
			...
			if (statusText.includes('hoàn thành') || $datlich_donvesinh.thoigian_batdau_thucte || $datlich_donvesinh.thoigian_ketthuc_thucte) {
		...
		async function handleStart() {
			const idDv = app.$datlich_donvesinh.id;
			const todayStr = (() => {
				const d = now();
				const p = n => String(n).padStart(2, '0');
				return `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())}`;
			...
			const totalDay = Math.max(1, diffDays(app.$datlich_donvesinh.ngay_bat_dau_kehoach, app.$datlich_donvesinh.ngay_ket_thuc_kehoach) + 1);
			const percentPerDay = 100 / totalDay;
			const nextProgress = Math.min(100, toNum(app.$datlich_donvesinh.tien_do || 0) + percentPerDay);
		...
		async function handleEnd() {
			...
			app.$workHistory = await listHistoryByOrder(app.$datlich_donvesinh.id);
		...
		async function handleSaveStaffLog() {
			...
			await insertRow(TABLE_HISTORY, {
				id_dv: app.$datlich_donvesinh.id,
				ngay_lam: todayStr,
		...
		async function checkAutoEndPreviousWorkDay() {
			if (!app.$datlich_donvesinh) return;
			const idDv = app.$datlich_donvesinh.id;
			...
			const autoEndTime = app.$datlich_donvesinh.gio_ket_thuc_kehoach || '23:59:59';
		...
		async function handleSaveCustomerReview() {
			...
			try {
				...
				const media = uploaded.length ? uploaded : parseMedia(app.$datlich_donvesinh.media_danhgia_khachhang);
				await updateById(TABLE_ORDER, app.$datlich_donvesinh.id, {
		...
		async function handleSaveStaffReview() {
			if (toNum(app.$datlich_donvesinh?.id_nhacungcap) !== toNum(app.$user?.id)) {
			...
			try {
				...
				const media = uploaded.length ? uploaded : parseMedia(app.$datlich_donvesinh.media_danhgia_nhanvien);
				await updateById(TABLE_ORDER, app.$datlich_donvesinh.id, {
		...
		async function init() {
			try {
				...
				renderUI();
				show($('mainGrid'), true);
				const statusText = String(app.$datlich_donvesinh?.trangthai || '').toLowerCase();
				const noStaff = toNum(app.$datlich_donvesinh?.id_nhacungcap) <= 0;
