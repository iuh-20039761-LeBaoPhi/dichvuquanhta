...
$data = get_donhang_data();
$rows = is_array($data['rows'] ?? null) ? $data['rows'] : [];
$error = (string) ($data['error'] ?? '');

// Lấy danh sách trạng thái có trong dữ liệu
$statusMap = [];
foreach ($rows as $row) {
    if (!is_array($row)) {
        continue;
...
admin_render_layout_start('Quản Lý Đơn Hàng Thuê Tài Xế', 'orders', $admin);
?>
        ...
        <form method="get" class="row g-2 align-items-end">
            <div class="col-12 col-md-4 col-lg-3">
                ...
                <input type="text" class="form-control" name="q" value="<?= admin_h($q) ?>"
                    placeholder="Mã đơn, tên KH, SĐT, điểm đến...">
            </div>
        ...
        <?php else: ?>
                ...
                <table class="table table-hover align-middle mb-0">
                    ...
                    <tbody>
                        <?php if (!$paginatedRows): ?>
                            <tr>
                                <td colspan="8" class="text-center py-4 text-secondary">Không có đơn hàng phù hợp.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($paginatedRows as $row): ?>
                                <?php $meta = donhang_status_meta((string) ($row['trangthai'] ?? '')); ?>
                                <tr>
                                    ...
                                    <td><?= admin_h(trim((string) ($row['diemden'] ?? '')) !== '' ? (string) $row['diemden'] : 'N/A') ?>
                                    </td>
                                    <td><?= number_format((float) ($row['tong_tien'] ?? 0), 0, ',', '.') ?> đ</td>
                                    <td><span
            ...
            <div class="order-list-mobile d-lg-none mt-2">
                ...
                <?php else: ?>
                    <?php foreach ($paginatedRows as $row): ?>
                        <?php $meta = donhang_status_meta((string) ($row['trangthai'] ?? '')); ?>
                        <a href="chi-tiet-don-hang.php?id=<?= urlencode((string) ($row['id'] ?? '')) ?>" class="order-card-mobile">
                            <div class="order-card-inner">
                                <div class="order-card-row">
                                    <span class="order-card-id">#<?= str_pad((string) ($row['id'] ?? ''), 7, '0', STR_PAD_LEFT) ?></span>
                                ...
                                <div class="order-card-row">
                                    <span class="order-card-customer"><?= admin_h(trim((string) ($row['tenkhachhang'] ?? '')) !== '' ? (string) $row['tenkhachhang'] : 'N/A') ?></span>
                                    <span class="order-card-status badge <?= admin_h((string) $meta['badge']) ?>"><?= admin_h((string) $meta['text']) ?></span>
                                </div>
                                <div class="order-card-service">
                                    <?= admin_h(trim((string) ($row['dich_vu'] ?? '')) !== '' ? (string) $row['dich_vu'] : 'N/A') ?>
                                </div>
                                <div class="order-card-destination">
                                    <i class="bi bi-geo-alt-fill me-1"></i><?= admin_h(trim((string) ($row['diemden'] ?? '')) !== '' ? (string) $row['diemden'] : 'N/A') ?>
                                </div>
                                <div class="order-card-price"><?= number_format((float) ($row['tong_tien'] ?? 0), 0, ',', '.') ?> đ</div>
                            </div>
                        </a>
                    <?php endforeach; ?>
                <?php endif; ?>
