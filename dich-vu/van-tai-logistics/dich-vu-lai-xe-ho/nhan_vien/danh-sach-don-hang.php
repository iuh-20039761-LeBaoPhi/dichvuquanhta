...
$summaryPending = count(array_filter($rows, static fn(array $i): bool => trim((string) ($i['trangthai'] ?? '')) === '' || trim((string) ($i['trangthai'] ?? '')) === 'chờ xác nhận'));
$summaryReceived = count(array_filter($rows, static fn(array $i): bool => 
    trim((string) ($i['trangthai'] ?? '')) === 'hoàn thành' || 
    trim((string) ($i['trangthai'] ?? '')) === 'đã nhận' || 
    trim((string) ($i['trangthai'] ?? '')) === 'đang thực hiện' ||
    trim((string) ($i['trangthai'] ?? '')) === 'đã xác nhận'
));
$summaryTotal = count($rows);
?>
<?php
$pageTitle = "Danh sách đơn hàng thuê tài xế";
...
<style>
    ...
    .jobs-table { --bs-table-hover-bg: #eef6ff; }
    ...
    .jobs-table tbody td { border-color: #e0edf8; }
    ...
    .invoice-card:active { background: #eef6ff; }
    .invoice-card .inv-id { color: #007bff; font-weight: 700; font-size: 0.95rem; }
    ...
    .invoice-card .inv-status { background: #1e88e5; color: #fff; font-size: 0.7rem; padding: 4px 10px; border-radius: 999px; font-weight: 600; }
    .invoice-card .inv-service { color: #64748b; font-size: 0.85rem; margin-top: 2px; }
    .invoice-card .inv-price { color: #dc3545; font-weight: 700; font-size: 1.1rem; text-align: right; margin-top: 4px; }
    .invoice-card .inv-destination { color: #6c757d; font-size: 0.8rem; margin-top: 2px; }
</style>
            ...
            <?php else: ?>
                <div class="d-flex flex-column flex-lg-row justify-content-between align-items-lg-center gap-2 mb-3">
                    <div>
                        ...
                        <p class="summary-note mb-0">Quản lý đơn hàng thuê tài xế</p>
                    </div>
                ...
                <div class="row g-2 g-lg-3 mb-3">
                    <div class="col-12 col-md-4">
                        <div class="stat-card">
                            <div class="d-flex align-items-center justify-content-between gap-3">
                                <div>
                                    <div class="text-secondary small">Chờ xác nhận</div>
                    ...
                    <div class="col-12 col-md-4">
                        <div class="stat-card">
                            <div class="d-flex align-items-center justify-content-between gap-3">
                                <div>
                                    <div class="text-secondary small">Đã nhận / Đang thực hiện</div>
                    ...
                    <div class="col-12 col-md-4">
                        <div class="stat-card">
                            <div class="d-flex align-items-center justify-content-between gap-3">
                                <div>
                                    <div class="text-secondary small">Tổng đơn hàng</div>
                ...
                <form method="get" class="filter-box mb-3">
                    <div class="row g-2 align-items-end">
                        <div class="col-12 col-md-4 col-lg-3">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control" name="q"
                                    ...
                                    placeholder="ID, khách hàng, SĐT, điểm đến...">
                            </div>
                        </div>
                        ...
                        <div class="col-6 col-md-3 col-lg-2 d-flex gap-2">
                            <button class="btn btn-primary flex-fill" type="submit"><i class="bi bi-funnel me-1"></i>Lọc</button>
                            <a class="btn btn-outline-secondary" href="danh-sach-don-hang.php"
                               onclick="event.preventDefault(); navigateTo('danh-sach-don-hang.php');"><i class="bi bi-arrow-counterclockwise"></i></a>
                        </div>
                    </div>
                </form>
                ...
                <div class="table-wrap">
                    ...
                    <div class="table-responsive d-none d-md-block">
                        <table class="table jobs-table align-middle">
                            ...
                            <tbody>
                                <?php if ($loadError !== ''): ?>
                                    <tr>
                                        <td colspan="8" class="empty-row py-4">Lỗi tải dữ liệu: <?= htmlspecialchars($loadError, ENT_QUOTES, 'UTF-8') ?></td>
                                    </tr>
                                <?php elseif (!$paginatedRows): ?>
                                    <tr>
                                        <td colspan="8" class="empty-row py-4">Không có đơn hàng phù hợp bộ lọc.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($paginatedRows as $item): ?>
                                        ...
                                        if ($statusValue === '') { $statusValue = 'chờ xác nhận'; }
                                        $badgeClass = 'text-bg-secondary';
                                        if ($statusValue === 'chờ xác nhận') { $badgeClass = 'text-bg-warning'; }
                                        elseif ($statusValue === 'đã xác nhận') { $badgeClass = 'text-bg-info'; }
                                        elseif ($statusValue === 'đã nhận' || $statusValue === 'đang thực hiện') { $badgeClass = 'text-bg-success'; }
                                        elseif ($statusValue === 'hoàn thành') { $badgeClass = 'text-bg-primary'; }
                                        ?>
                                        <tr>
                                            ...
                                            <td>
                                                <div class="action-group">
                                                    <a href="chi-tiet-don-hang.html?mahd=<?= urlencode((string) $itemId) ?>&sodienthoai=<?= urlencode((string) ($_SESSION['user']['sodienthoai'] ?? '')) ?>&password=<?= urlencode((string) ($_SESSION['user']['matkhau'] ?? '')) ?>"
                                                        onclick="if(typeof navigateTo === 'function') { navigateTo(this.getAttribute('href')); return false; }"
                                                        class="btn btn-primary btn-action"><i class="bi bi-eye"></i>Chi tiết</a>
                                                </div>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                    ...
                    <div class="d-md-none">
                        ...
                        <?php elseif (!$paginatedRows): ?>
                            <div class="empty-row py-4">Không có đơn hàng.</div>
                        <?php else: ?>
                            <?php foreach ($paginatedRows as $item): ?>
                                <?php
                                $itemId = (int) ($item['id'] ?? 0);
                                ...
                                $detailUrl = "chi-tiet-don-hang.html?mahd=" . urlencode((string) $itemId) . "&sodienthoai=" . urlencode((string) ($_SESSION['user']['sodienthoai'] ?? '')) . "&password=" . urlencode((string) ($_SESSION['user']['matkhau'] ?? ''));
                                ?>
                                <a href="<?= $detailUrl ?>" class="invoice-card"
                                    ...
                                    <div class="inv-destination"><i class="bi bi-geo-alt-fill me-1"></i><?= htmlspecialchars((string) ($item['diemden'] ?? 'N/A'), ENT_QUOTES, 'UTF-8') ?></div>
                ...
                <?php if ($loadError === '' && $totalFiltered > 0): ?>
                    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-2 mt-3">
                        <?php if ($totalPages > 1): ?>
                            <nav aria-label="Phân trang đơn hàng nhân viên" class="w-100">
                                <ul class="pagination pagination-sm mb-0 justify-content-center">
                                    <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                                        <a class="page-link" href="<?= htmlspecialchars($buildPageUrl(max(1, $page - 1)), ENT_QUOTES, 'UTF-8') ?>"
                                           onclick="event.preventDefault(); navigateTo(this.getAttribute('href'));">Trước</a>
                                    </li>
