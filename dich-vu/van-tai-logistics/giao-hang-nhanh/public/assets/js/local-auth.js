(function (window, document) {
  ...
  function mapSharedUserRecord(row) {
    ...
    const avatarLink = normalizeText(
      row.link_avatar || row.avatar_link || row.avatar || "",
    );
    const cccdFrontLink = normalizeText(
      row.link_cccd_truoc || row.cccd_front_link || "",
    );
    const cccdBackLink = normalizeText(
      row.link_cccd_sau || row.cccd_back_link || "",
    );
    const avatarName = normalizeText(
      row.avatartenfile || row.avatar_name || "",
    );
    const cccdFrontName = normalizeText(
      row.cccdmattruoctenfile || row.cccd_front_name || "",
    );
    const cccdBackName = normalizeText(
    ...
    return {
      ...
      link_avatar: avatarLink,
      avatar_link: avatarLink,
      link_cccd_truoc: cccdFrontLink,
      cccd_front_link: cccdFrontLink,
      link_cccd_sau: cccdBackLink,
      cccd_back_link: cccdBackLink,
      avatar_name: avatarName,
      avatartenfile: avatarName,
      cccd_front_name: cccdFrontName,
      cccdmattruoctenfile: cccdFrontName,
      cccd_back_name: cccdBackName,
      cccdmatsautenfile: cccdBackName,
  ...
  async function updateKrudUser(userId, role, patch = {}) {
    ...
    const mappings = {
      ...
      link_avatar: normalizedPatch.link_avatar ?? normalizedPatch.avatar_link,
      link_cccd_truoc:
        normalizedPatch.link_cccd_truoc ?? normalizedPatch.cccd_front_link,
      link_cccd_sau:
        normalizedPatch.link_cccd_sau ?? normalizedPatch.cccd_back_link,
    };
