<?php
$pageTitle = "Chi tiết đơn hàng Chăm sóc người già";
...
<style>
	...
	* {
	...
	:root {
		...
		--danger: #1b5e20;
		--border: #c8e6c9;
		--shadow: 0 20px 45px rgba(46, 125, 50, 0.18);
		--accent-peach: #d4e157;
		--accent-lavender: #c8e6c9;
		--accent-mint: #cff5e8;
		--accent-rose: #a5d6a7;
	}
	...
	.panel {
		border-color: #c8e6c9;
		box-shadow: 0 12px 26px rgba(46, 125, 50, 0.12), 0 2px 8px rgba(191, 200, 219, 0.1);
	}
	...
	.invoice-hero {
		background: linear-gradient(118deg, #c8e6c9 0%, #e8f5e9 48%, #f1f8f1 72%, #f1f8f1 100%);
	}
	...
	.appDialogBtnPrimary {
		background: linear-gradient(135deg, #79e788, #11c23d);
		color: white;
		...
		<div class="content">
			...
			<section id="mainGrid" class="grid hidden">
				...
				<article class="panel" id="panelTime"
					style="background:linear-gradient(180deg,#2e7d32 0%,#f1f8f1 100%);border-color:#c8e6c9;">
					...
					<div style="display:grid;gap:6px;">
						...
						<div
							style="width:100%;height:21px;border-radius:999px;overflow:hidden;background:linear-gradient(180deg,#e8f5e9 0%,#ffffff 100%);border:1px solid #2e7d32;box-shadow:inset 0 1px 2px rgb(255, 255, 255);">
					...
					<div
						...
						<div
							style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));background:#66bb6a;color:#ffffff;font-size:12px;font-weight:700;">
					...
					<div class="d-flex align-items-center flex-wrap" style="gap:8px;">
						...
						<div class="actions d-flex align-items-center ms-auto" style="gap:8px;">
							...
							<span id="startButtonCountdown" class="hidden"
								style="font-size:11px;font-weight:700;color:#1b5e20;background:#e8f5e9;border:1px solid #c8e6c9;border-radius:999px;padding:4px 8px;white-space:nowrap;">--</span>
					...
					<div style="border:1px solid #d3efd4;border-radius:8px;overflow:hidden;background:#f7fff7;">
						<div
							style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));background:#66bb6a;color:#ffffff;font-size:12px;font-weight:700;">
							...
							<div
								...
								<div class="d-flex justify-content-between align-items-center"
									style="gap:8px;padding:7px 10px;font-size:12px;"><span
										...
										style="color:#1b5e20;font-weight:700;">---</span></div>
								<div class="d-flex justify-content-between align-items-center"
									...
									<span style="color:#000000;font-weight:700;">Kết thúc</span><span id="timeEndPlan"
										style="color:#1b5e20;font-weight:700;">---</span>
								</div>
							</div>
					...
					<div id="workHistoryTable" style="margin-top:4px;">
						...
						<p id="workHistoryEmpty" style="font-size:12px;color:#388e3c;margin:0;">Chưa có lịch sử làm
							việc.</p>
					</div>
				...
				<article class="panel" id="panelCustomer">
					...
					<div class="profile-body">
						<img id="customerAvatarImg" class="profile-avatar" src="assets/logong.png" alt="Khách hàng">
				...
				<article class="panel" id="panelStaff">
					...
					<div class="profile-body">
						<img id="staffAvatarImg" class="profile-avatar" src="assets/logong.png" alt="Nhà Cung Cấp">
...
<script>
	(() => {
		...
		const TABLE_ORDER = 'datlich_nguoigia';
		const TABLE_HISTORY = 'lich_su_lam_viec_nguoigia';
		...
		const app = {
			...
			$datlich_nguoigia: null,
			stateTimer: null,
			// id của record lich_su_lam_viec_nguoigia đang mở trong ngày (sau khi nhấn btnStart)
		...
		const setAvatar = (id, src, fallback = '../assets/logong.png') => {
			const el = $(id);
			if (!el) return;
			el.onerror = () => {
				el.onerror = null;
		...
		async function applyAutoOrderStatusRules($datlich_nguoigia) {
			const status = String($datlich_nguoigia.trangthai || '').toLowerCase();
			const hasStaff = toNum($datlich_nguoigia.id_nhacungcap) > 0;
			const startPlanAt = buildPlanDateTime($datlich_nguoigia.ngay_bat_dau_kehoach, $datlich_nguoigia.gio_bat_dau_kehoach);
			const endPlanAt = buildPlanDateTime($datlich_nguoigia.ngay_ket_thuc_kehoach, $datlich_nguoigia.gio_ket_thuc_kehoach, true);
			...
			if (needOverdue) {
				await updateById(TABLE_ORDER, $datlich_nguoigia.id, { trangthai: 'quá hạn' });
				return { ...$datlich_nguoigia, trangthai: 'quá hạn' };
			}

			if (needComplete) {
				await updateById(TABLE_ORDER, $datlich_nguoigia.id, { trangthai: 'hoàn thành' });
				return { ...$datlich_nguoigia, trangthai: 'hoàn thành' };
			}

			return $datlich_nguoigia;
		}

		async function loadOrderById() {
			const $datlich_nguoigia_rows = await listTableRows(TABLE_ORDER);
			const $datlich_nguoigia = $datlich_nguoigia_rows.find((row) => String(row.id) === String(app.params.mahd));
			if (!$datlich_nguoigia) throw new Error('Không tìm thấy đơn hàng');

			app.$datlich_nguoigia = await applyAutoOrderStatusRules($datlich_nguoigia);
		}

		function getAssignedStaffRow() {
			if (!app.$datlich_nguoigia) return null;
			const idStaff = toNum(app.$datlich_nguoigia.id_nhacungcap);
			if (idStaff <= 0) return null;
			// Tìm trong bảng nguoidung (thay thế cho nhacungcap_nguoigia cũ)
		...
		function calcProgressDay($datlich_nguoigia) {
			const plan = parseTimeToHour($datlich_nguoigia.gio_ket_thuc_kehoach) - parseTimeToHour($datlich_nguoigia.gio_bat_dau_kehoach);
			const realStart = new Date($datlich_nguoigia.thoigian_batdau_thucte || '').getTime();
			const realEnd = new Date($datlich_nguoigia.thoigian_ketthuc_thucte || '').getTime();
			const real = (realEnd - realStart) / 3600000;
			const days = diffDays($datlich_nguoigia.ngay_bat_dau_kehoach, $datlich_nguoigia.ngay_ket_thuc_kehoach);
		...
		function isReviewLockedByDeadline($datlich_nguoigia) {
			if (!$datlich_nguoigia) return true;
			const status = String($datlich_nguoigia.trangthai || '').toLowerCase();
			const endPlanAt = buildPlanDateTime($datlich_nguoigia.ngay_ket_thuc_kehoach, $datlich_nguoigia.gio_ket_thuc_kehoach, true);
			...
			const isCanceled = status.includes('hủy');
			const isOverdue = status.includes('quá hạn');
			const noStaff = toNum($datlich_nguoigia.id_nhacungcap) <= 0;
			return now().getTime() > endPlanAt.getTime() && (isCanceled || isOverdue || noStaff);
		}

		function canSubmitCustomerReview() {
			const $datlich_nguoigia = app.$datlich_nguoigia;
			if (!$datlich_nguoigia) return false;
			const isCompleted = String($datlich_nguoigia.trangthai || '').toLowerCase().includes('hoàn thành');
			return isCompleted && !isReviewLockedByDeadline($datlich_nguoigia);
		}
		...
		function canSubmitStaffReview() {
			...
			if (!$datlich_nguoigia) return false;
			const isCompleted = String($datlich_nguoigia.trangthai || '').toLowerCase().includes('hoàn thành');
			const isAssignedStaff = toNum($datlich_nguoigia.id_nhacungcap) > 0
				&& toNum($datlich_nguoigia.id_nhacungcap) === toNum(app.$user?.id);
			return isCompleted && isAssignedStaff && !isReviewLockedByDeadline($datlich_nguoigia);
		}
		...
		function bindReviewPreview(inputId, gridId, mediaColumn) {
			...
			input.addEventListener('change', () => {
				...
				renderMedia(gridId, (app.$datlich_nguoigia || {})[mediaColumn]);
			});
		}

		function renderReview() {
			const $datlich_nguoigia = app.$datlich_nguoigia;
			if (!$datlich_nguoigia) return;
			const customerDone = !!$datlich_nguoigia.danhgia_khachhang;
			const staffDone = !!$datlich_nguoigia.danhgia_nhanvien;

			setText('customerReviewText', $datlich_nguoigia.danhgia_khachhang || 'Chưa có đánh giá');
			setText('customerReviewTime', formatDateTimeDisplay($datlich_nguoigia.thoigian_danhgia_khachhang));
			setText('staffReviewText', $datlich_nguoigia.danhgia_nhanvien || 'Chưa có đánh giá');
			setText('staffReviewTime', formatDateTimeDisplay($datlich_nguoigia.thoigian_danhgia_nhanvien));
			renderMedia('customerReviewMediaGrid', $datlich_nguoigia.media_danhgia_khachhang);
			renderMedia('staffReviewMediaGrid', $datlich_nguoigia.media_danhgia_nhanvien);
		...
		function renderUI() {
			const $datlich_nguoigia = app.$datlich_nguoigia || {};
			const $nhacungcap_nguoigia = getAssignedStaffRow() || {};
			const invoiceCode = formatInvoiceCode($datlich_nguoigia.id || app.params.mahd);

			setText('invoiceId', invoiceCode);
			setText('invoicePrice', formatMoney($datlich_nguoigia.tong_tien || 0));
			setText('invoiceService', $datlich_nguoigia.dich_vu || '---');
			setText('invoiceAddress', $datlich_nguoigia.diachikhachhang || '---');
			setText('invoiceTimeRange', `${formatTimeDisplay($datlich_nguoigia.gio_bat_dau_kehoach)} - ${formatTimeDisplay($datlich_nguoigia.gio_ket_thuc_kehoach)}`);
			setText('invoiceDate', `${formatDateDisplay($datlich_nguoigia.ngay_bat_dau_kehoach)}${$datlich_nguoigia.ngay_ket_thuc_kehoach ? ` -> ${formatDateDisplay($datlich_nguoigia.ngay_ket_thuc_kehoach)}` : ''}`);
			setText('invoiceRequest', $datlich_nguoigia.yeu_cau_khac || '---');
			setText('invoiceNote', $datlich_nguoigia.ghi_chu || '---');
			renderJobList($datlich_nguoigia.cong_viec || '');
			...
			renderDriveFrame($datlich_nguoigia.anh_id || '', 'invoiceMediaImageEl', 'invoiceMediaImageEmpty');
			renderDriveFrame($datlich_nguoigia.video_id || '', 'invoiceMediaVideoEl', 'invoiceMediaVideoEmpty');

			setText('customerCardName', $datlich_nguoigia.tenkhachhang || '---');
			setText('customerCardPhone', $datlich_nguoigia.sdtkhachhang || '---');
			setText('customerCardEmail', $datlich_nguoigia.emailkhachhang || '---');
			setText('customerCardAddress', $datlich_nguoigia.diachikhachhang || '---');
			...
			updateAvatarIframe('customerAvatar', $datlich_nguoigia.avatar_khachhang);

			setText('staffCardName', $datlich_nguoigia.tenncc || '---');
			setText('staffCardEmail', $datlich_nguoigia.emailncc || '---');
			setText('staffCardPhone', $datlich_nguoigia.sdtncc || '---');
			setText('staffCardAddress', $datlich_nguoigia.diachincc || '---');
			setText('staffReceiveTime', `Nhận việc: ${formatDateTimeDisplay($datlich_nguoigia.ngaynhan)}`);
			updateAvatarIframe('staffAvatar', $datlich_nguoigia.avatar_ncc);

			const status = $datlich_nguoigia.trangthai || '---';
			...
			setText('timeStartPlan', `${formatTimeDisplay($datlich_nguoigia.gio_bat_dau_kehoach)}`.trim());
			setText('timeEndPlan', `${formatTimeDisplay($datlich_nguoigia.gio_ket_thuc_kehoach)}`.trim());
			setText('timeStartReal', formatDateTimeDisplay($datlich_nguoigia.thoigian_batdau_thucte));
			setText('timeEndReal', formatDateTimeDisplay($datlich_nguoigia.thoigian_ketthuc_thucte));

			const progress = Math.max(0, Math.min(100, toNum($datlich_nguoigia.tien_do || 0)));
			...
			const totalDayCount = Math.max(1, diffDays($datlich_nguoigia.ngay_bat_dau_kehoach, $datlich_nguoigia.ngay_ket_thuc_kehoach) + 1);
			...
			setText('dateStartPlan', formatDateDisplay($datlich_nguoigia.ngay_bat_dau_kehoach));
			setText('dateEndPlan', formatDateDisplay($datlich_nguoigia.ngay_ket_thuc_kehoach));
			...
			$('badgeStaff').textContent = toNum($datlich_nguoigia.id_nhacungcap) > 0 ? 'Đã nhận' : 'Chưa nhận';
			$('badgeStaff').className = toNum($datlich_nguoigia.id_nhacungcap) > 0 ? 'profile-status badge success' : 'profile-status badge warning';
		...
		function renderWorkHistoryTable() {
			...
			const endPlanTime = app.$datlich_nguoigia ? app.$datlich_nguoigia.gio_ket_thuc_kehoach : '';
		...
		async function updateCustomerAvatarIfNeeded() {
			const order = app.$datlich_nguoigia;
			const user = app.$user;
		...
		async function handleClaim() {
			const $datlich_nguoigia = app.$datlich_nguoigia || {};
			const statusText = String($datlich_nguoigia.trangthai || '').toLowerCase();
			...
			if (startPlanAt && now().getTime() >= startPlanAt.getTime()) {
				if (toNum($datlich_nguoigia.id_nhacungcap) <= 0 && !statusText.includes('hủy') && !statusText.includes('hoàn thành')) {
					await updateById(TABLE_ORDER, $datlich_nguoigia.id, { trangthai: 'quá hạn' });
					await reloadOrder();
				}
				throw new Error('Đơn hàng quá hạn, không thể nhận việc');
			...
			await updateById(TABLE_ORDER, app.$datlich_nguoigia.id, {
		...
		async function handleCancel() {
			...
			if (startPlanAt && now().getTime() >= startPlanAt.getTime()) {
				if (!hasStaff && !statusText.includes('hoàn thành')) {
					await updateById(TABLE_ORDER, app.$datlich_nguoigia.id, { trangthai: 'quá hạn' });
					await reloadOrder();
			...
			if (statusText.includes('hoàn thành') || $datlich_nguoigia.thoigian_batdau_thucte || $datlich_nguoigia.thoigian_ketthuc_thucte) {
		...
		// handleStart: bắt đầu làm trong ngày - insert mới vào lich_su_lam_viec_nguoigia
		async function handleStart() {
			const idDv = app.$datlich_nguoigia.id;
			const todayStr = (() => {
				const d = now();
				const p = n => String(n).padStart(2, '0');
				return `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())}`;
			...
			const totalDay = Math.max(1, diffDays(app.$datlich_nguoigia.ngay_bat_dau_kehoach, app.$datlich_nguoigia.ngay_ket_thuc_kehoach) + 1);
			const percentPerDay = 100 / totalDay;
			const nextProgress = Math.min(100, toNum(app.$datlich_nguoigia.tien_do || 0) + percentPerDay);
		...
		async function handleEnd() {
			...
			app.$workHistory = await listHistoryByOrder(app.$datlich_nguoigia.id);
		...
		async function checkAutoEndPreviousWorkDay() {
			if (!app.$datlich_nguoigia) return;
			const idDv = app.$datlich_nguoigia.id;
			...
			const autoEndTime = app.$datlich_nguoigia.gio_ket_thuc_kehoach || '23:59:59';
		...
		async function handleSaveCustomerReview() {
			...
			try {
				...
				const media = uploaded.length ? uploaded : parseMedia(app.$datlich_nguoigia.media_danhgia_khachhang);
				await updateById(TABLE_ORDER, app.$datlich_nguoigia.id, {
		...
		async function handleSaveStaffReview() {
			if (toNum(app.$datlich_nguoigia?.id_nhacungcap) !== toNum(app.$user?.id)) {
			...
			try {
				...
				const media = uploaded.length ? uploaded : parseMedia(app.$datlich_nguoigia.media_danhgia_nhanvien);
				await updateById(TABLE_ORDER, app.$datlich_nguoigia.id, {
		...
		async function init() {
			try {
				...
				renderUI();
				show($('mainGrid'), true);
				const statusText = String(app.$datlich_nguoigia?.trangthai || '').toLowerCase();
				const noStaff = toNum(app.$datlich_nguoigia?.id_nhacungcap) <= 0;
