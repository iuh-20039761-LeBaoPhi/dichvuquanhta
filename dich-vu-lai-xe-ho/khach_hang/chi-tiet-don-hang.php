...
function taixe_parse_media_paths(string $raw): array
{
    $raw = trim($raw);
    if ($raw === '') {
        return [];
...
function taixe_media_is_video(string $path): bool
{
...
$sessionUser = session_user_require_customer('../login.html', 'khach_hang/chi-tiet-don-hang.php' . (isset($_GET['id']) ? ('?id=' . urlencode((string)$_GET['id'])) : ''));
$sessionPhone = (string)($sessionUser['sodienthoai'] ?? '');

$invoiceId = (int)($_GET['id'] ?? 0);
$result = getDonHangBySessionSdt($sessionPhone, $invoiceId > 0 ? $invoiceId : null);
$invoice = $result['row'] ?? null;
$loadError = (string)($result['error'] ?? '');

if ($invoiceId <= 0) {
    $loadError = 'Thiếu mã đơn hàng để hiển thị chi tiết.';
}
if ($invoiceId > 0 && !$invoice && $loadError === '') {
    $loadError = 'Không tìm thấy đơn hàng hoặc bạn không có quyền xem đơn hàng này.';
}
...
$invoiceCode = taixe_format_invoice_id_display($invoice['id'] ?? '');
...
$progress = 0;
if (!empty($invoice['ngay_bat_dau_kehoach']) && !empty($invoice['ngay_ket_thuc_kehoach'])) {
    $start = strtotime($invoice['ngay_bat_dau_kehoach']);
    $end = strtotime($invoice['ngay_ket_thuc_kehoach']);
    $now = time();
    if ($now >= $end) {
        $progress = 100;
    } elseif ($now <= $start) {
...
$totalMoneyFormatted = number_format((float)$totalMoneyText, 0, ',', '.') . ' đ';
...
$pickupAddress = trim((string)($invoice['diemdon'] ?? ''));
if ($pickupAddress === '') {
    $pickupAddress = '---';
}

$destination = trim((string)($invoice['diemden'] ?? ''));
if ($destination === '') {
    $destination = '---';
}

$distanceKm = trim((string)($invoice['quangduong_dukien'] ?? ''));
if ($distanceKm === '') {
    $distanceKm = '---';
} else {
...
$driverAssigned =
    ...
    || (int)($invoice['id_taixe'] ?? 0) > 0;

$cancelCheck = taixe_can_cancel_invoice($invoice);
...
$customerReviewMedia = taixe_parse_media_paths((string)($invoice['media_danhgia_khachhang'] ?? ''));
$customerReviewHasData = $customerReviewText !== '' || $customerReviewTime !== '' || $customerReviewMedia !== [];

$driverReviewText = trim((string)($invoice['danhgia_taixe'] ?? ''));
$driverReviewTime = trim((string)($invoice['thoigian_danhgia_taixe'] ?? ''));
$driverReviewMedia = taixe_parse_media_paths((string)($invoice['media_danhgia_taixe'] ?? ''));
$driverReviewHasData = $driverReviewText !== '' || $driverReviewTime !== '' || $driverReviewMedia !== [];

$reviewCheck = taixe_can_customer_review($invoice);
...
<head>
    ...
    <title>Chi Tiết Đơn Hàng Thuê Tài Xế</title>
    ...
    <style>
        ...
        .hero-box {
            padding: 10px;
            background: linear-gradient(95deg, #007bff 0%, #00b4d8 100%);
            color: #fff;
        }
...
<body>
<?php render_khach_hang_header($sessionUser, 'Chi tiết đơn hàng', 'orders'); ?>
<div class="page-wrap">
    <?php if ($flashMsg !== ''): ?>
            ...
            <div class="hero-box">
                ...
                <div class="hero-stats">
                    <article class="hero-stat">
                        ...
                        <p class="value"><?= htmlspecialchars($totalMoneyFormatted, ENT_QUOTES, 'UTF-8') ?></p>
                    </article>
                    <article class="hero-stat">
                    ...
                    <article class="hero-stat">
                        <p class="label"><i class="bi bi-geo-alt"></i>Điểm đón</p>
                        <p class="value"><?= htmlspecialchars($pickupAddress, ENT_QUOTES, 'UTF-8') ?></p>
                    </article>
                </div>
            </div>
            ...
            <div class="content-grid">
                <article class="panel">
                    <div class="panel-head">
                        <h2 class="panel-title">Thông tin chuyến đi</h2>
                    </div>
                    <div class="jobs-meta" style="grid-template-columns: 1fr;">
                        <div class="jobs-meta-item">
                            <p class="label-xs">Điểm đến</p>
                            <p class="value-sm"><?= htmlspecialchars($destination, ENT_QUOTES, 'UTF-8') ?></p>
                        </div>
                        <div class="jobs-meta-item">
                            <p class="label-xs">Quãng đường dự kiến</p>
                            <p class="value-sm"><?= htmlspecialchars($distanceKm, ENT_QUOTES, 'UTF-8') ?></p>
                        </div>
                        <div class="jobs-meta-item">
                            <p class="label-xs">Yêu cầu khác</p>
                ...
                <article class="panel">
                    <div class="panel-head">
                        <h2 class="panel-title" style="font-size:1.2rem;">Trạng thái và tiến độ</h2>
                    </div>
                    <div class="progress-block">
                        ...
                        <p class="muted-note">Tiến độ cộng dồn theo từng ngày.</p>
                        ...
                        <p class="muted-note">Số ngày: <?= (int)$daysPlan ?> ngày</p>
                        <?php if ($canCancel && $idNumber > 0): ?>
                            <form method="post" action="xu-ly-huy.php" onsubmit="return confirm('Bạn có chắc muốn hủy đơn này không?');">
                                ...
                                <input type="hidden" name="return_to" value="<?= htmlspecialchars('chi-tiet-don-hang.php?id=' . $idNumber, ENT_QUOTES, 'UTF-8') ?>">
                ...
                <article class="panel">
                    <div class="panel-head">
                        ...
                        <span class="chip success">Thông tin</span>
                    </div>
                    <div class="person-card">
                        <div class="person-head">
                            <img class="avatar" src="../<?= htmlspecialchars(trim((string)($invoice['avatar_khachhang'] ?? '')) !== '' ? (string)$invoice['avatar_khachhang'] : '../assets/logo_main.png', ENT_QUOTES, 'UTF-8') ?>" alt="avatar khách hàng">
                        ...
                        <div class="person-foot">
                            <span class="chip">Ngày đặt: <?= htmlspecialchars(trim((string)($invoice['ngaydat'] ?? '')) !== '' ? (string)$invoice['ngaydat'] : '---', ENT_QUOTES, 'UTF-8') ?></span>
                        </div>
                    </div>
                </article>
                ...
                <article class="panel">
                    <div class="panel-head">
                        <h2 class="panel-title" style="font-size:1.8rem;">Tài xế phụ trách</h2>
                        <span class="chip <?= $driverAssigned ? 'success' : 'warning' ?>"><?= $driverAssigned ? 'Đã phân công' : 'Chưa phân công' ?></span>
                    </div>
                    <div class="person-card">
                        <div class="person-head">
                            <img class="avatar" src="../<?= htmlspecialchars(trim((string)($invoice['avatar_taixe'] ?? '')) !== '' ? (string)$invoice['avatar_taixe'] : '../assets/logo_main.png', ENT_QUOTES, 'UTF-8') ?>" alt="avatar tài xế">
                            <h3 class="person-name"><?= htmlspecialchars(trim((string)($invoice['ten_taixe'] ?? '')) !== '' ? (string)$invoice['ten_taixe'] : '---', ENT_QUOTES, 'UTF-8') ?></h3>
                        </div>
                        <div class="person-items">
                            <p class="person-row"><i class="bi bi-telephone-fill"></i><span><?= htmlspecialchars(trim((string)($invoice['sdt_taixe'] ?? '')) !== '' ? (string)$invoice['sdt_taixe'] : '---', ENT_QUOTES, 'UTF-8') ?></span></p>
                            <p class="person-row"><i class="bi bi-card-list"></i><span>Kinh nghiệm: <?= htmlspecialchars(trim((string)($invoice['kinh_nghiem_taixe'] ?? '')) !== '' ? (string)$invoice['kinh_nghiem_taixe'] : '---', ENT_QUOTES, 'UTF-8') ?> năm</span></p>
                        </div>
                        <div class="person-foot">
                            <span class="chip">Số bằng lái: <?= htmlspecialchars(trim((string)($invoice['so_bang_lai'] ?? '')) !== '' ? (string)$invoice['so_bang_lai'] : '---', ENT_QUOTES, 'UTF-8') ?></span>
                            <span class="chip">Hạng bằng: <?= htmlspecialchars(trim((string)($invoice['hang_bang_lai'] ?? '')) !== '' ? (string)$invoice['hang_bang_lai'] : '---', ENT_QUOTES, 'UTF-8') ?></span>
                        </div>
                    </div>
                </article>
                ...
                <article class="panel panel-full">
                    <div class="panel-head">
                        <h2 class="panel-title" style="font-size:1.45rem;">Đánh giá</h2>
                        <span class="chip">Phản hồi</span>
                    </div>
                    <div class="review-wrap">
                        <section class="review-box">
                            ...
                            <div class="review-body">
                                ...
                                <p class="label-xs">Hình ảnh</p>
                                <div class="media-grid">
                                    <?php if (!$customerReviewMedia): ?>
                                        <div class="media-empty">Chưa có hình ảnh</div>
                                    <?php else: ?>
                                        <?php foreach ($customerReviewMedia as $mediaPath): ?>
                                            <?php if (taixe_media_is_video($mediaPath)): ?>
                                                <video controls preload="metadata" src="<?= htmlspecialchars($mediaPath, ENT_QUOTES, 'UTF-8') ?>"></video>
                                            <?php else: ?>
                                                <img src="<?= htmlspecialchars($mediaPath, ENT_QUOTES, 'UTF-8') ?>" alt="Ảnh đánh giá">
                                            <?php endif; ?>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </div>
                                ...
                                <?php if ($canReview && $idNumber > 0): ?>
                                    <form method="post" action="xu-ly-huy.php" enctype="multipart/form-data" class="mt-2 d-grid gap-2">
                                        ...
                                        <input type="hidden" name="invoice_id" value="<?= (int)$idNumber ?>">
                                        <input type="hidden" name="return_to" value="<?= htmlspecialchars('chi-tiet-don-hang.php?id=' . $idNumber, ENT_QUOTES, 'UTF-8') ?>">
                                        <textarea name="review_text" class="form-control form-control-sm" rows="3" placeholder="Nhập đánh giá của bạn về dịch vụ"><?= htmlspecialchars($customerReviewText, ENT_QUOTES, 'UTF-8') ?></textarea>
                                        <input type="file" name="review_media[]" class="form-control form-control-sm" accept="image/*,video/*" multiple>
                                        <div>
                                            <button type="submit" class="btn btn-primary btn-sm"><i class="bi bi-save me-1"></i>Gửi đánh giá</button>
                                        </div>
                                    </form>
                        ...
                        <section class="review-box">
                            <div class="review-head">
                                <h3 class="review-title">Đánh giá của tài xế</h3>
                                <span class="chip <?= $driverReviewHasData ? 'success' : 'warning' ?>"><?= $driverReviewHasData ? 'Đã có' : 'Chưa có' ?></span>
                            </div>
                            <div class="review-body">
                                <p class="label-xs">Nội dung đánh giá</p>
                                <p class="review-text"><?= htmlspecialchars($driverReviewText !== '' ? $driverReviewText : 'Chưa có đánh giá', ENT_QUOTES, 'UTF-8') ?></p>
                                <p class="label-xs">Thời gian gửi</p>
                                <p class="review-text"><?= htmlspecialchars($driverReviewTime !== '' ? $driverReviewTime : '---', ENT_QUOTES, 'UTF-8') ?></p>
                                <p class="label-xs">Hình ảnh</p>
                                <div class="media-grid">
                                    <?php if (!$driverReviewMedia): ?>
                                        <div class="media-empty">Chưa có hình ảnh</div>
                                    <?php else: ?>
                                        <?php foreach ($driverReviewMedia as $mediaPath): ?>
                                            <?php if (taixe_media_is_video($mediaPath)): ?>
                                                <video controls preload="metadata" src="<?= htmlspecialchars($mediaPath, ENT_QUOTES, 'UTF-8') ?>"></video>
                                            <?php else: ?>
                                                <img src="<?= htmlspecialchars($mediaPath, ENT_QUOTES, 'UTF-8') ?>" alt="Ảnh đánh giá">
