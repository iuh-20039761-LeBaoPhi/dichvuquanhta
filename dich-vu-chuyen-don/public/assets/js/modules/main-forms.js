    ...
    pricingReferencePromise = fetch(
      ...
      .then((pricingData) => {
        bookingVehicleOptions =
          buildBookingVehicleOptionsFromPricingData(pricingData);
        setBookingVehicleLabelMap(
...
async function uploadSelectedBookingMedia(scope) {
  ...
  if (videoFiles.length) {
    try {
      ...
      console.error(
        "Không thể tải video đặt lịch chuyển dọn lên Drive:",
        error,
      );
...
async function createBookingRequest(scope, portalStore) {
  ...
  if (!hasMatchingCustomerSession) {
    ...
    accountSetup =
      await authService.ensureCustomerAccountForBooking(bookingContact);
  }
