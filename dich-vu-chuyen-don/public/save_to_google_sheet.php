...
if (!is_array($payload)) {
    echo json_encode(['success' => false, 'message' => 'Dữ liệu gửi sheet không hợp lệ'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}

$sheetType = isset($payload['sheet_type']) ? trim((string) $payload['sheet_type']) : '';
if ($sheetType === '') {
    echo json_encode(['success' => false, 'message' => 'Thiếu trường sheet_type'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}

$jsonBody = json_encode($payload, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
if ($jsonBody === false) {
    echo json_encode(['success' => false, 'message' => 'Không thể mã hóa dữ liệu gửi sheet'], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}
...
if ($response === false) {
    echo json_encode(['success' => false, 'message' => 'Lỗi kết nối Google Sheet: ' . $curlErr], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    exit;
}
